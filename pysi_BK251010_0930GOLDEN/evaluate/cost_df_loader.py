...
def build_cost_df_from_sql(db_path: str) -> pd.DataFrame:
    ...
    return out
# --- COMPAT SHIM: legacy API kept for old GUI imports ------------------------
def build_cost_df_from_csv_dir(csv_dir: str | None = None, **kwargs):
    """
    [COMPAT] 旧GUIが import だけしている関数。基本は SQL ローダに委譲します。
    - 優先: kwargs['db_path'] / 環境変数 PSI_DB_PATH / ./var/psi.sqlite
    - 本関数は import エラー回避が主目的。CSV-only で使いたい場合は
      アップサート実行の上、 build_cost_df_from_sql() をお使いください。
    """
    import os
    import pandas as pd  # noqa: F401  (将来CSV直読みを実装する場合に備えて)
    # 1) DB パスを解決
    db_path = (kwargs.get("db_path")
               or os.environ.get("PSI_DB_PATH")
               or os.path.join(os.getcwd(), "var", "psi.sqlite"))
    # 2) DB があれば SQL ローダに委譲
    if os.path.isfile(db_path):
        print(f"[COMPAT] build_cost_df_from_csv_dir -> using SQL loader: {db_path}")
        return build_cost_df_from_sql(db_path)
    # 3) ここまで来たら CSV-only を本当に使いたいケース。
    #    ただし現在の実装は SQL スキーマに依存しているため、明示メッセージで誘導。
    raise ImportError(
        "build_cost_df_from_csv_dir is deprecated. "
        "Please upsert CSVs into the DB and call build_cost_df_from_sql(db_path). "
        "Hints:\n"
        ...
        "- Then use build_cost_df_from_sql('./var/psi.sqlite')."
    )
# -----------------------------------------------------------------------------
# ====== COMPAT SHIM: normalize_cost_df_columns ===============================
# 既存モジュールに RATIO_COLS/_validate_sum_r が無い場合のフォールバック定義
try:
    RATIO_COLS
except NameError:
    RATIO_COLS = [
...
def _validate_sum_r_fallback(df, ratio_cols, policy: str = "warn"):
    import numpy as np
    import pandas as pd
    sum_r = df[ratio_cols].sum(axis=1).astype(float)
    mask = sum_r > 1.0 + 1e-9
    if not mask.any():
        return df
    if {"product_name","node_name"} <= set(df.columns):
        bad_pairs = df.loc[mask, ["product_name","node_name"]].astype(str)\
                      .agg(" / ".join, axis=1).tolist()
    else:
        bad_pairs = df.index[mask].astype(str).tolist()
    msg = f"[sum_r check] >1.0 at {mask.sum()} rows: {bad_pairs}"
    ...
    if policy == "error":
        ...
        over = df.loc[mask, ratio_cols]
        denom = over.sum(axis=1).replace(0, np.nan)
        df.loc[mask, ratio_cols] = over.div(denom, axis=0).fillna(0.0)
        print(f"[FIXED] {mask.sum()} rows were scaled to sum_r==1.0. {msg}", flush=True)
    else:
        print(f"[WARNING] {msg}", flush=True)
    return df
def normalize_cost_df_columns(df: pd.DataFrame,
                              policy: str | None = None,
                              ratio_cols: list[str] | None = None) -> pd.DataFrame:
    """
    コスト“比率”列を 0..1 に正規化し、合計>1.0 の行を policy に従って処理。
      - policy: 'warn' | 'scale' | 'error'（未指定時は環境変数 PSI_COST_SUM_POLICY を参照）
      - ratio_cols: 明示指定が無ければ RATIO_COLS ∩ df.columns を使用（無ければ RATIO_COLS を新規0列で補完）
    戻り値: 正規化済み DataFrame（破壊的でない）
    """
    import numpy as np, os, pandas as pd
    df = df.copy()
    # 対象列を決定し、無い列は0で追加
    base = ratio_cols or [c for c in RATIO_COLS if c in df.columns]
    if not base:
        base = RATIO_COLS[:]  # 何も無ければ標準一式を対象にする
    for c in base:
        if c not in df.columns:
            df[c] = 0.0
    # % → 0..1 に統一
    def to_ratio(s: pd.Series) -> pd.Series:
        x = pd.to_numeric(s, errors="coerce").fillna(0.0)
        x = np.where(x <= 1.0, x, x / 100.0)
        return pd.Series(np.clip(x, 0.0, 1.0), index=s.index)
    for c in base:
        df[c] = to_ratio(df[c])
    # 合計チェック
    pol = (policy or os.environ.get("PSI_COST_SUM_POLICY", "warn")).lower()
    if "_validate_sum_r" in globals():
        return _validate_sum_r(df, ratio_cols=base, policy=pol)
    else:
        return _validate_sum_r_fallback(df, ratio_cols=base, policy=pol)
# =============================================================================
